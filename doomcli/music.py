...
soundtrack is reproduced here: the map's music lump is read out of the WAD and
looped. On Windows it goes through the built-in MIDI sequencer (MCI/winmm),
just like the original game. On Linux/macOS there is no such built-in synth and
pygame/SDL_mixer can't play MIDI, so the lump is rendered once to WAV by the
fluidsynth synthesizer (cached) and looped through pygame.mixer.music. Either
way, pausing the game pauses the tune.
"""
import atexit
import ctypes
import hashlib
...
# Windows' MCI (winmm) is what plays the MIDI soundtrack; it doesn't exist on
# POSIX, so MusicPlayer.start() degrades to False there and the game simply
# plays without music.
_winmm = None
...
def _mci(cmd: str) -> str:
    if _winmm is None:
        raise OSError("MCI (winmm) is not available on this platform")
...
class PygameMusicPlayer:
    """POSIX soundtrack: the WAD's MIDI lump rendered to WAV by fluidsynth
    and looped through pygame.mixer.music.

    pygame/SDL_mixer 2.x can't play MIDI itself, so the map's real MIDI is
    synthesized once by the fluidsynth command (cached per track), and the
    resulting WAV is looped with pygame's mature mixer — pausing the game
    pauses the tune, exactly like the Windows MCI path.

    Requires: fluidsynth + a soundfont (Ubuntu: sudo apt install fluidsynth
    fluid-soundfont-gm). pygame ships as a vizdoom dependency.
    """
    ...
    _SOUNDFONTS = (
        ...
        "/opt/homebrew/share/soundfonts/FluidR3_GM.sf2", # macOS (Apple Silicon)
        "/usr/local/share/soundfonts/FluidR3_GM.sf2",    # macOS (Intel)
    ...
    def _render(self, midi: bytes) -> str | None:
        """Synthesize a MIDI lump to a cached WAV via fluidsynth."""
        digest = hashlib.sha256(midi).hexdigest()[:16]
        cache_dir = os.path.join(tempfile.gettempdir(), "doomcli-music")
        os.makedirs(cache_dir, exist_ok=True)
        wav = os.path.join(cache_dir, f"{digest}.wav")
        if os.path.exists(wav) and os.path.getsize(wav) > 0:
            return wav
        soundfont = next((p for p in self._SOUNDFONTS if os.path.exists(p)),
                         None)
        if not soundfont:
            return None
        mid = os.path.join(cache_dir, f"{digest}.mid")
        with open(mid, "wb") as f:
            f.write(midi)
        try:
            subprocess.run(
                ["fluidsynth", "-ni", "-g", "1.0", "-T", "wav",
                 "-F", wav, soundfont, mid],
