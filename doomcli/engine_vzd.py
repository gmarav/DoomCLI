...
if not _IS_WINDOWS:
    # Render fully headless on POSIX: SDL creates no real window at all
    # (set_window_visible(False) is only honored after init, and without this
    # the native window would flash on screen once at startup). Video only —
    # the engine's OpenAL sound is unaffected.
...
def _suppress_engine_window(stop: threading.Event) -> None:
    """Hide ViZDoom's window the moment it's created (Windows only).
    ...
    on screen. Shove it off-screen and hide it as soon as it exists. On POSIX
    SDL_VIDEODRIVER=dummy avoids a real window entirely, so this is skipped.
    """
    import win32con
    import win32gui
...
class DoomEngine:
    def __init__(self, wad: int = 1, doom_map: str = "E1M1", skill: int = 3,
        ...
        # On POSIX, Ubuntu's OpenAL 1.19 EFX extension is broken with ViZDoom
        # (init succeeds but audio is silent) — disabling EFX fixes it
        # (Farama-Foundation/ViZDoom#486). Windows OpenAL is fine, so EFX stays.
        game_args = "+win_x -32000 +win_y -32000 +snd_sfxvolume 1"
        if not _IS_WINDOWS:
            game_args += " +snd_efx 0"
        g.add_game_args(game_args)
